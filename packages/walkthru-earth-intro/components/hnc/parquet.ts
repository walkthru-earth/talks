  ...
  const rows: HNCRow[] = (out as Record<string, unknown>[])
    .map((r) => {
      ...
      return {
        ...
        lon: coords ? coords[0] : Number.NaN,
        lat: coords ? coords[1] : Number.NaN,
        top_regions: Array.isArray(r.top_regions) ? (r.top_regions as RegionScore[]) : [],
      }
    })
    .filter(r => Number.isFinite(r.lon) && Number.isFinite(r.lat))
  rows.sort((a, b) => rowTimestamp(a.captured_at) - rowTimestamp(b.captured_at))
  return { rows, file }
}

export async function loadParquetHeavy(file: AsyncBuffer): Promise<Map<string, HNCHeavy>> {
  const out = await parquetReadObjects({
    file,
  ...
  for (const raw of out as Record<string, unknown>[]) {
    ...
    const ba = raw.brain_activity ? Float32Array.from(raw.brain_activity as ArrayLike<number>) : null
