...
import { FSAVERAGE5_HEMI_VERTS, HNC_GLB, HNC_PARCEL_ATLAS_URL, type SurfaceMode } from './config'
...
export function loadParcelAtlas(url = HNC_PARCEL_ATLAS_URL): Promise<ParcelAtlas | null> {
...
export async function loadHemispheres(surface: SurfaceMode): Promise<BrainAssembly> {
  ...
  const sides: Array<{ side: Hemi, mesh: THREE.Mesh | null, lowMesh: THREE.Mesh | null }> = [
  ...
  for (const { side, mesh, lowMesh } of sides) {
    ...
    const lowPos = lowMesh.geometry.attributes.position.array as ArrayLike<number>
    ...
    const highPos = mesh.geometry.attributes.position.array as ArrayLike<number>
    const nHigh = mesh.geometry.attributes.position.count
    const mapping = new Int32Array(nHigh)
    for (let i = 0; i < nHigh; i++) {
      mapping[i] = grid.nearest(highPos[i * 3], highPos[i * 3 + 1], highPos[i * 3 + 2])
    }
