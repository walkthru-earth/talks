// Assets are bundled with the deck under public/hnc and served from a relative
// path so the talk runs fully offline. BASE_URL resolves to "/" in dev and to
// the deploy base (for example /talks/walkthru-earth-auc-p1/) in the build, so
// the same code works in both. The map basemap still streams from Carto, see
// HNC_MAP_STYLE below.
export const HNC_BASE = `${import.meta.env.BASE_URL}hnc`
